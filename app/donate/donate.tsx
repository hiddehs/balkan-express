...
export default function Donate () {
  ...
  return (
    <div>
      <div
        className={`md:rounded-2xl bg-midnight-800 select-none text-center text-3xl font-display font-bold w-full p-6 py-10 md:p-10 mt-8 md:rounded-lg ${depr
          ? 'block'
          : 'hidden'}`}>
        The Express has arrived at its destination 🚂
      </div>
      <form id={'donate'} action={(data) => startTransition(async () => {
        await handleSubmit(data)
      })} onClick={() => setEmailVisible(true)}
            className={`md:rounded-2xl bg-midnight-800 select-none text-center w-full p-6 py-10 md:p-10 mt-8 md:rounded-lg  ${depr
              ? 'hidden'
              : 'block'} ${isPending
              ? 'opacity-40'
              : 'opacity-100'} `}>

        <Image src={'/beer.png'} alt={'beer'} className="mx-auto mb-2"
               width={98}
               height={500}></Image>
        <div
          className={`flex-col gap-4 ${step === 1
            ? 'inline-flex'
            : 'hidden'}`}>
          <div
          ...
          <p className="text-dessert-500 mb-4">We&apos;ll put this name on the
            car
            ✨</p>
          {names.length > 0 ?
        ...
        <div
          className={`flex-col gap-4 ${step === 0
            ? 'inline-flex'
            : 'hidden'}`}>
